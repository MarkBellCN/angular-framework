  ...
  template     : `
    <ul [class.ant-table-pagination]="nzInTable" class="ant-pagination ant-pagination-simple" *ngIf="nzSimple">
      ...
      <li [attr.title]="_current+'/'+_lastIndex" class="ant-pagination-simple-pager">
        <input type="number" [ngModel]="nzPageIndex" (ngModelChange)="_nzPageIndexChange($event)" size="3"><span class="ant-pagination-slash">／</span>{{ _lastIndex }}
      </li>
      <li
      ...
      <div class="ant-pagination-options">
        ...
        <div class="ant-pagination-options-quick-jumper"
          *ngIf="nzShowQuickJumper">
          {{ 'Pagination.jumpTo' | nzTranslate }}<input #jumperInput [ngModel]="nzPageIndex" (ngModelChange)="_nzPageIndexChange($event,jumperInput)">{{ 'Pagination.page' | nzTranslate }}
        </div>
      </div>
    </ul>`,
  styleUrls    : [
...
export class NzPaginationComponent {
  ...
  _nzPageIndexChange($event: number,jumperInput:any): void {
    if(!isNaN(Number($event))){
      this.nzPageIndex = $event;
      this.nzPageIndexChange.emit(this.nzPageIndex);
    }else{
      jumperInput.value=this._current;
    }
  }
