...
export class NzTreeService {
   ...
   * @param {any[]} root
   */
  initCheckedStatus(childNode: NzTreeNode, defaultCheckedKeys: string[], nzInitCheckStrictly:boolean): void {
    ...
    if (childNode.getChildren()) {
      childNode.getChildren().forEach((child) => {
        ...
        if (!nzInitCheckStrictly && childNode.isChecked && !child.isDisabled && !child.isDisableCheckbox) {
          child.isChecked = true;
          child.isAllChecked = true;
          child.isHalfChecked = false;
        }
        this.initCheckedStatus(child, defaultCheckedKeys, nzInitCheckStrictly);
      });
    }
  }

  initTreeNodes(root: NzTreeNode[], defaultCheckedKeys: string[] = [], nzCheckStrictly: boolean = false, nzInitCheckStrictly:boolean =false): NzTreeNode[] {
    this.rootNodes = root;
    if (root.length > 0) {
      root.forEach((node) => {
        this.initCheckedStatus(node, defaultCheckedKeys, nzInitCheckStrictly);
      });
      if (!nzCheckStrictly) {
        root.forEach((node) => {
          this.initParentNode(node);
        });
        // should uptpsd node status
      }
    }
  ...
  // uptpsd all nodes to unselected
