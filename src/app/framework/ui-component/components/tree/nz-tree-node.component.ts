...
export class NzTreeNodeComponent implements OnInit, AfterViewInit {
  ...
  @Input() nzCheckStrictlyChild: boolean;
  ...
  _clickCheckBox($event: MouseEvent, node: NzTreeNode): void {
    ...
    if (this.nzCheckStrictly) {
      ...
      if(!this.nzCheckStrictlyChild){
        ...
        this.nzTreeService.setCheckedNodeListStrict(this.nzTreeNode);
        //向下级联选择
        this.nzTreeService.checkTreeNodeChildren(node, node.isChecked);
        if(node.isChecked){
          this.nzTreeService.checkTreeNodeParents(node);
        }
      }
