  ...
  template : `
    <ul
      ...
      <nz-tree-node *ngFor="let node of ngModelNodes" [nzTreeNode]="node"
                    ...
                    [nzCheckStrictlyChild]="nzCheckStrictlyChild"
...
export class NzTreeComponent implements OnInit {
  ...
  @Input() _ngModelNodes: NzTreeNode[] = [];
  @ContentChild('nzTreeTemplate') nzTreeTemplate: TemplateRef<{}>;

  @Input() nzCheckStrictly: boolean = false;
  @Input() nzInitCheckStrictly: boolean = false;
  @Input() nzCheckStrictlyChild: boolean = false;
  @Input() nzCheckable;
  ...
  @Input() get ngModelNodes(): NzTreeNode[] {
    return this._ngModelNodes;
  }
  set ngModelNodes(ngModelNodes: NzTreeNode[]) {
    if (ngModelNodes) {
      this._ngModelNodes = ngModelNodes;
      this.nzTreeService.initTreeNodes(this.ngModelNodes, this.nzDefaultCheckedKeys, this.nzCheckStrictly,this.nzInitCheckStrictly);
      this.onChange(ngModelNodes);
      this.ngModelNodesChange.emit(ngModelNodes);
    }
  }
  ...
  @Output() ngModelNodesChange: EventEmitter<any> = new EventEmitter();
